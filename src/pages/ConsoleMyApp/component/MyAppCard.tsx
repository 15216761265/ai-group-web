...
const CardCom: React.FC<{ data: IMyAppList }> = ({ data }) => {
  ...
  return (
    <Card hoverable className="ai-group-console-myapp-card">
...
const MyAppCard = () => {
  const { myAppList } = useContext(MyAppContext);
  return (
    <div className="ai-group-console-myapp-card-list">
      {myAppList?.map((list, index) => (
        <CardCom key={index} data={list} />
      ))}
      <Card className="ai-group-console-myapp-card h-[166px]" hoverable>
        <div className="flex items-center ">
